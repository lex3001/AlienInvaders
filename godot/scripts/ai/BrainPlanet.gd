...
func reset_brain_state() -> void:
	...
	if actor and level:
		...
		# Add buffer on sides so planet is fully visible and reachable by player
		var buffer = 40.0
		var x_pos = randf_range(buffer, level.play_width - SIZE_PLANET().x - buffer)
	...
	if state == State.HAS_NOT_APPEARED and ticks_ms > 0.0:
		...
		if ticks_passed >= ticks_before_appearance:
			...
			if level.has_method("play_sound"):
				level.play_sound("PLANET_APPEAR")
