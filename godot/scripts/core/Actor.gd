...
func _draw() -> void:
	# Allow brain to draw custom elements (e.g., AlienF laser)
	if brain != null and brain.has_method("draw_custom"):
		brain.draw_custom(self)
...
func play_animation(anim_name: String, start_random: bool = false, next_name: String = "") -> void:
	if not anim_sequences.has(anim_name):
		return
	if anim_name.find("explode") != -1 and has_meta("hit_position"):
		position = get_meta("hit_position") as Vector2
		movement_type = MovementType.NORMAL
		velocity = Vector2.ZERO
		velocity_magnitude = 0.0
		acceleration = 0.0
		leader = null
		always_update_radial_position = false
